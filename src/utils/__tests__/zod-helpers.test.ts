...
describe('Zod Helpers', () => {
  describe('getFirstZodError', () => {
    ...
    it('returns error for specific field path', () => {
      ...
      if (!result.success) {
        ...
        expect(usernameError).toBe('Too small: expected string to have >=3 characters')
  ...
  describe('getAllZodErrors', () => {
    it('returns all error messages', () => {
      ...
      if (!result.success) {
        ...
        expect(errors).toContain('Too small: expected string to have >=8 characters')
      }
    })
  ...
  describe('zodErrorsToFieldMap', () => {
    it('converts errors to field-error map', () => {
      ...
      if (!result.success) {
        ...
        expect(errorMap.username).toBe('Too small: expected string to have >=3 characters')
  ...
  describe('createZodFieldValidator', () => {
    it('validates a field schema directly', () => {
      const validator = createZodFieldValidator(z.string().min(3, 'Username is too short'))
  ...
  describe('validateAsync', () => {
    ...
    it('works with refine async', async () => {
      ...
      if (!invalidResult.success) {
        expect(invalidResult.error.issues[0].message).toBe('Username is already taken')
      }
    })
  })
  ...
  describe('commonSchemas', () => {
    ...
    describe('mustBeTrue', () => {
      ...
      it('has custom error message', () => {
        const result = commonSchemas.mustBeTrue.safeParse(false)
        if (!result.success) {
          expect(result.error.issues[0].message).toBe('You must accept to continue')
        }
      })
    })
  })
  ...
  describe('createPasswordMatchSchema', () => {
    ...
    it('supports custom field names', () => {
      ...
      expect(result.success).toBe(false)
      if (!result.success) {
        const error = result.error.issues.find((e) => e.path[0] === 'newPasswordConfirm')
